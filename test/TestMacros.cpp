...
void TestMacros::testMethodWithCStringArg(char *arg) { TEST_ASSERT(strlen(arg) == 2); }
...
void TestMacros::testMethodWithStringArg(std::string string) {
  ...
  TEST_BIPREDICATE_MSG(
      [](std::string s1, std::string s2) { return s1.size() != s2.size(); }, string, string, "Will fail");
}
...
void TestMacros::throwsException() { throw std::invalid_argument("Some invalid argument"); }

void TestMacros::throwsInt() { throw 5; }

void TestMacros::testMethodWithVarargs1(const double d, const int i) { TEST_ASSERT_MSG(d == i, "Fails for d != i"); }
