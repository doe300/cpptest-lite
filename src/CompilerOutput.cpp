...
void CompilerOutput::printFailure(const Assertion &assertion) {
  ...
  if (index != std::string::npos)
    formatString.replace(
        index, ARG_TEXT.size(), (!assertion.errorMessage.empty() ? assertion.errorMessage : assertion.userMessage));
  stream << formatString << std::endl;
}

void CompilerOutput::printException(const std::string &suiteName, const std::string &methodName,
